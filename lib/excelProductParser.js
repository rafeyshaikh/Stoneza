...
export function parseProductExcel(input) {
  ...
  if (typeof input === "string") {
    workbook = readFile(input);
  } else {
    workbook = read(input, { type: "buffer" });
  }
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    // Format Images (Only accept valid URLs)
    const images = isValidUrl(mainImgLink)
      ? [{ url: mainImgLink, publicId: "" }]
      : [];

    const hoverImage = isValidUrl(hoverImgLink)
