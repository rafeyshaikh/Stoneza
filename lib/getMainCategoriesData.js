  ...
  try {
    ...
    for (const cat of orderedTopCats) {
      ...
      result.push({
        ...
        image:
          cat.bannerImage?.square?.url ||
          cat.bannerImage?.wide?.url ||
          (Array.isArray(cat.bannerImage?.wide) ? cat.bannerImage.wide[0]?.url : "") ||
          "",
        countLabel: `${level2.length} SUB-CATEGORIES · ${level3Count} SERIES`,
        href: `/product-category/${cat.slug}`,
      });
    }

    // 4th Card: Collections (4 Master Families, 18 Named Series)
    const totalSeriesCount = await Collection.countDocuments({ collectionLevel: 2, isActive: { $ne: false } });
    ...
    result.push({
      ...
      countLabel: `4 FAMILIES · ${totalSeriesCount || 18} SERIES`,
      href: "/collections",
    });
