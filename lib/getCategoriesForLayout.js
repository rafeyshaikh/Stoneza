    ...
    const normalized = tree.map((cat) => {
      ...
      const dbColumns =
        cat.children?.map((sub) => ({
          ...
          squareImage: sub.squareBanner?.url || sub.wideBanners?.[0]?.url || "",
          links:
            sub.children?.map((third) => {
              ...
              return {
                ...
                squareImage: third.squareBanner?.url || third.wideBanners?.[0]?.url || "",
              };
            }) || [],
        })) || [];
          ...
          if (existingCol) {
            ...
            (cmsCol.links || []).forEach((cmsLink) => {
              ...
              if (!linkExists) {
                existingCol.links.push({
                  ...
                  href:
                    cmsLink.href ||
                    `/categories/${cmsLink.slug || cmsLink.name.toLowerCase().replace(/ /g, "-")}`,
                });
              }
            });
          } else {
            megamenuColumns.push({
              ...
              links: (cmsCol.links || []).map((link) => ({
                ...
                href:
                  link.href ||
                  `/categories/${link.slug || link.name.toLowerCase().replace(/ /g, "-")}`,
              })),
            });
          }
        });
      ...
      if (cat.megamenu?.featuredCard?.title) {
        featuredCard = {
          ...
          href: cat.megamenu.featuredCard.href || `/categories/${cat.slug}`,
        };
      }
