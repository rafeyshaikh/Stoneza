...
import { parseMasterProductCSV, slugify } from "../lib/csvMasterProductParser.js";
...
async function runSeed() {
  try {
    ...
    const csvPath = path.join(process.cwd(), "public", "Stoneza_MASTER_Product_Sheet_v6 (2).csv");
    console.log(`Parsing Master CSV from: ${csvPath}`);

    const { categoryTree, products } = parseMasterProductCSV(csvPath);
    ...
    for (const prodData of products) {
      ...
      const existingProduct = await Product.findOne({ slug: rest.slug });
      if (existingProduct) {
